...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("Togetzr Support".to_owned());
    static ref KEY_PAIR: Mutex<Option<KeyPair>> = Default::default();
    static ref USER_DEFAULT_CONFIG: RwLock<(UserDefaultConfig, Instant)> = RwLock::new((UserDefaultConfig::load(), Instant::now()));
    pub static ref NEW_STORED_PEER_CONFIG: Mutex<HashSet<String>> = Default::default();
    pub static ref DEFAULT_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    // For "official-like" branded builds, embed and lock server settings here so the UI shows
    // them prefilled and non-editable.
    pub static ref OVERWRITE_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new({
        let mut m: HashMap<String, String> = HashMap::new();
        // Togetzr Support embedded servers
        m.insert(
            keys::OPTION_CUSTOM_RENDEZVOUS_SERVER.to_string(),
            "remote.2getzr.com:21116".to_owned(),
        );
        m.insert(
            keys::OPTION_RELAY_SERVER.to_string(),
            "remote.2getzr.com:21117".to_owned(),
        );
        m.insert(keys::OPTION_API_SERVER.to_string(), "".to_owned());
        m.insert(
            keys::OPTION_KEY.to_string(),
            "cM9AH+IEZcpQIFv4RkY45AG3IcX128ZJXJ9Dh6NB4Ac=".to_owned(),
        );
        m
    });
    ...
    // Built-in settings are not user-editable. We use them to make sure this branded build
    // does not try to use an API server when "API Server" is intentionally left empty.
...
pub const LINK_DOCS_HOME: &str = "https://togetzr.com/docs/en/";
pub const LINK_DOCS_X11_REQUIRED: &str = "https://togetzr.com/docs/en/manual/linux/#x11-required";
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["remote.2getzr.com"];
pub const RS_PUB_KEY: &str = "cM9AH+IEZcpQIFv4RkY45AG3IcX128ZJXJ9Dh6NB4Ac=";
