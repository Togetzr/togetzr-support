...
pub fn check_software_update() {
    // Togetzr Support: disable update checks entirely (no network call).
    if get_app_name() == "Togetzr Support" {
        return;
    }
    if is_custom_client() {
        return;
...
// Togetzr Support: update checks are disabled.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_RUSTDESK_CLIENT.to_string());
    // Safety: if update URL is empty or disabled, skip network call.
...
/// Returns the executable filename (with `.exe`) used for Windows install/uninstall/shortcuts.
///
/// This is intentionally **not** derived from `get_app_name()` because `get_app_name()` may
/// contain spaces (e.g. "Togetzr Support"), while the executable filename is often an identifier
/// style name (e.g. "Togetzr_Support.exe").
#[inline]
pub fn get_app_exe_filename() -> String {
    #[cfg(windows)]
    {
        // Keep this consistent with the actual Windows runner output filename.
        // (The filesystem is case-insensitive, but we keep a stable casing for registry/shortcuts.)
        if get_app_name() == "Togetzr Support" {
            return "Togetzr_Support.exe".to_owned();
        }
        return format!("{}.exe", get_app_name());
    }
    #[cfg(not(windows))]
    {
        // Non-windows callers typically don't use this.
        get_app_name()
    }
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    // Fallback only. For this fork, runtime communication is hosted under 2getzr.com.
    "https://2getzr.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    url.contains("rustdesk.com/") || url.ends_with("rustdesk.com") ||
    url.contains("togetzr.com/") || url.ends_with("togetzr.com") ||
    url.contains("2getzr.com/") || url.ends_with("2getzr.com")
}
