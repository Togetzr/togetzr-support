...
pub fn run(rx: Receiver<RecordState>) {
    std::thread::spawn(move || {
        ...
        // API is intentionally disabled for this build (API Server left empty).
        // Avoid constructing relative URLs like "/api/login-options".
        if api_server.trim().is_empty() || crate::is_public(&api_server) {
            log::info!("record upload disabled: api server is empty/disabled");
            return;
        }
